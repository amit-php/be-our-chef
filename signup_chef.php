<?php
	...
	$usertype  =  "chef";
	$spaclization  =  $data['spaclization'];
	...
	if(!empty($fullName) && !empty($email_id) && !empty($password)){
			...
			if(!email_exists($email_id)){
				...
				if($user_signup && !is_wp_error($user_signup)){
						...
						update_user_meta($user_signup, '_location', $location);
						update_user_meta($user_signup, '_specializations', $spaclization);
						update_user_meta($user_signup, '_radius', $radius);
						update_user_meta($user_signup, '_lat', $lat);
						update_user_meta($user_signup, '_lang', $lan);
